...
const Profile = () => {
  ...
  const fetchProfile = async () => {
    const { data } = await supabase.from("profiles").select("display_name, avatar_url, bio").eq("user_id", user!.id).maybeSingle();
  ...
  const fetchOrders = async () => {
    const { data } = await supabase
      .from("orders")
      .select("id, sale_id, game_name, package_amount, package_currency, final_price, player_id, status, created_at")
      .eq("user_id", user!.id)
  ...
  const handleSaveProfile = async () => {
    ...
    if (error) { toast.error("Failed to update profile."); } else { toast.success("Profile updated!"); setEditing(false); fetchProfile(); }
  };
            ...
            <div className="flex gap-2">
              {(["orders", "settings"] as const).map((tab) => (
                <button key={tab} onClick={() => setActiveTab(tab)}
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 ${activeTab === tab ? "btn-glow text-primary-foreground" : "glass-card text-muted-foreground hover:text-foreground"}`}
                >
            ...
            {activeTab === "orders" && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
                {orders.length === 0 ? (
                  ...
                  orders.map((order) => (
                    <motion.div key={order.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-4 space-y-2">
                      ...
                      <div className="flex items-center justify-between text-xs text-muted-foreground">
                        ...
                        <span className="font-bold text-primary">{order.final_price.toLocaleString()} E£</span>
                      </div>
                      <div className="flex items-center justify-between text-xs text-muted-foreground">
                ...
                <div className="glass-card p-4 space-y-4">
                  ...
                  {editing ? (
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2">
                        <button onClick={handleSaveProfile} className="flex-1 py-2 rounded-xl btn-glow text-sm font-semibold text-primary-foreground">Save</button>
                        <button onClick={() => setEditing(false)} className="flex-1 py-2 rounded-xl glass-card text-sm font-semibold text-muted-foreground hover:text-foreground">Cancel</button>
                      </div>
                    </div>
                  ) : (
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between"><span className="text-muted-foreground">Name</span><span className="text-foreground">{profile?.display_name || "—"}</span></div>
                      ...
                      <div className="flex justify-between"><span className="text-muted-foreground">Bio</span><span className="text-foreground">{profile?.bio || "—"}</span></div>
                    </div>
                  )}
                </div>
              </motion.div>
